...
const Complaint_schema_1 = require("../src/models/schema/Complaint.schema");
...
async function migrateComplaintStages() {
    ...
    try {
        ...
        const complaints = await Complaint_schema_1.ComplaintModel.find({
        ...
        for (const complaint of complaints) {
            ...
            try {
                await Complaint_schema_1.ComplaintModel.updateOne({ _id: complaint._id }, { $set: { stage: newStage } });
                successCount++;
...
async function migrateFieldNames() {
    ...
    try {
        ...
        const addressResult = await db.collection("complaints").updateMany({ address: { $exists: true }, addressId: { $exists: false } }, { $rename: { address: "addressId" } });
        console.log(`✅ Renamed 'address' -> 'addressId': ${addressResult.modifiedCount} documents`);
        // Rename 'parent' to 'parentId'
        const parentResult = await db.collection("complaints").updateMany({ parent: { $exists: true }, parentId: { $exists: false } }, { $rename: { parent: "parentId" } });
        console.log(`✅ Renamed 'parent' -> 'parentId': ${parentResult.modifiedCount} documents`);
        // Rename 'device' to 'deviceId'
        const deviceResult = await db.collection("complaints").updateMany({ device: { $exists: true }, deviceId: { $exists: false } }, { $rename: { device: "deviceId" } });
        ...
        const deviceTypeResult = await db.collection("complaints").updateMany({ deviceType: { $exists: true }, deviceTypeId: { $exists: false } }, { $rename: { deviceType: "deviceTypeId" } });
        console.log(`✅ Renamed 'deviceType' -> 'deviceTypeId': ${deviceTypeResult.modifiedCount} documents`);
        // Add default values for new fields
        const defaultsResult = await db.collection("complaints").updateMany({ notes: { $exists: false } }, {
            $set: {
                notes: "",
                media: [],
                subscriptionId: null,
...
//# sourceMappingURL=migrate-complaint-stages.js.map