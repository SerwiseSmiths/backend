...
exports.notifyComplaintCreated = notifyComplaintCreated;
exports.notifyComplaintUpdated = notifyComplaintUpdated;
exports.notifySubscriptionCreated = notifySubscriptionCreated;
exports.notifyWaitlistJoin = notifyWaitlistJoin;
...
const line = (emoji, label, value) => value !== undefined && value !== null && value !== ""
...
async function fetchUser(userId) {
    try {
        const u = await User_schema_1.default.findById(userId).select("firstName lastName phoneNo").lean();
        if (!u)
            return null;
        const name = `${u.firstName ?? ""} ${u.lastName ?? ""}`.trim() || "Unknown";
        return { name, phone: u.phoneNo ?? "—" };
    }
    catch {
        return null;
    }
...
const userLines = (u, fallbackId) => u
...
async function fetchAddress(addressId) {
    try {
        const a = await Address_scema_1.default.findById(addressId)
...
async function notifyNewUser(user) {
    ...
    const html = `🆕 <b>New User Registered</b>\n` +
        `${"─".repeat(28)}\n` +
        line("👤", "Name", name) +
...
async function notifyComplaintCreated(complaint) {
    ...
    const html = `🔧 <b>New Complaint Created</b>\n` +
        `${"─".repeat(28)}\n` +
...
async function notifyComplaintUpdated(complaint, changes) {
    ...
    const html = `🔄 <b>Complaint Updated</b>\n` +
        `${"─".repeat(28)}\n` +
        line("🆔", "Complaint ID", complaint._id?.toString()) +
        line("📌", "Title", complaint.title) +
        userLines(u, complaint.user) +
...
async function notifySubscriptionCreated(sub) {
    const snap = sub.plan_snapshot ?? {};
    const pricing = snap.pricing ?? {};
    const addons = (sub.addons_snapshot ?? []).map((a) => a.name).join(", ") || "None";
    const u = await fetchUser(sub.user);
    const html = `💳 <b>New Subscription Registered</b>\n` +
        `${"─".repeat(28)}\n` +
...
async function notifyWaitlistJoin(params) {
    const { phoneNo, countryCode, source, joinedAt } = params;
    const html = `📋 <b>New Waitlist Sign-up</b>\n` +
        `${"─".repeat(28)}\n` +
...
async function notifyPaymentVerified(params) {
    ...
    const snap = sub.plan_snapshot ?? {};
    const u = await fetchUser(sub.user);
    const html = `💰 <b>Payment Verified — Subscription Activated</b>\n` +
        `${"─".repeat(28)}\n` +
        line("🆔", "Subscription ID", sub._id?.toString()) +
        userLines(u, sub.user) +
        line("📦", "Plan", snap.name) +
