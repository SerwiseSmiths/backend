...
const retrieve = async (id) => {
    const dev = await (0, device_repo_1.retrieveDeviceById)(id, true);
    if (!dev)
...
const retrieveAll = async () => {
    ...
    const list = await (0, device_repo_1.retrieveAllDevices)(true);
...
const update = async (id, data) => {
    ...
    if (!dev)
        throw new ApiError_api_util_1.default(404, "Device not found");
    const updated = await (0, device_repo_1.updateDeviceById)(id, data, true);
...
const getDevicesByUser = async (userId) => {
    const devices = await (0, device_repo_1.retrieveDevicesByUserId)(userId, true);
    return devices;
};
