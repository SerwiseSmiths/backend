...
async function retrieveSubscriptionsByUser(userId) {
    const subs = await subscription_schema_1.SubscriptionModel.find({ user: userId })
        .populate("payments")
        .sort({ createdAt: -1 });
...
async function addPayment(_id, paymentId) {
    const subscription = await subRepo.retrieveSubscriptionById(_id);
    if (!subscription)
        throw new ApiError_api_util_1.default(400, "Invalid subscription");
    const updated = await subscription_schema_1.SubscriptionModel.findByIdAndUpdate(_id, { $push: { payments: paymentId } }, { new: true }).populate("payments");
    if (!updated)
        throw new ApiError_api_util_1.default(400, "Failed to add payment");
    return new ApiSuccess_api_util_1.default(200, "Payment added to subscription", updated);
}
/** GET SUBSCRIPTION PAYMENTS */
async function getSubscriptionPayments(_id) {
    const subscription = await subscription_schema_1.SubscriptionModel.findById(_id).populate("payments");
    if (!subscription)
        throw new ApiError_api_util_1.default(400, "Invalid subscription");
    return new ApiSuccess_api_util_1.default(200, "Subscription payments retrieved", {
        subscription,
